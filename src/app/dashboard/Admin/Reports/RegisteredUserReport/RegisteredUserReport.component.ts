...
@Component({
  selector: 'app-RegisteredUserReport',
  templateUrl: './RegisteredUserReport.component.html',
  styleUrls: ['./RegisteredUserReport.component.scss']
})
export class RegisteredUserReportComponent implements OnInit {
...
getReportResults(date) {
  if(Number(this.form.controls.role.value) != undefined && Number(this.form.controls.role.value) != null && Number(this.form.controls.role.value) != 0)
  {
    ...
    this.reportService.RegisteredUsersReport(Number(this.form.controls.role.value),date).subscribe((res:any)=>{
      console.log(res);
      let Months = [];
      let numbers = [];
